...
(begin-for-test
  ...
  (check-equal?
   (lolof=?
    (all-itineraries "LGA" "PDX" cyclicFlights '())
    (list
     (list
      (make-flight "Delta 0121" "LGA" "MSP" (make-UTC 11 0) (make-UTC 14 9))
      (make-flight "Delta 1609" "MSP" "DEN" (make-UTC 20 35) (make-UTC 22 52))
     ...
     (list
      ...
      (make-flight "Delta 2163" "MSP" "PDX" (make-UTC 15 0) (make-UTC 19 2)))))
   true
   "mis-match on all itineraries from LGA to PDX on cyclicFlights"))
...
(define (other-itineraries ap1 ap2 lof tlof rlof)
  (let ((ntlof (append tlof (list (first rlof)))))
    (if (check-flight-taken? (arrives (first rlof)) ntlof)
        (find-other-itineraries ap1 ap2 lof tlof (rest rlof))
        (append (all-itineraries (arrives (first rlof)) ap2 lof ntlof)
                (find-other-itineraries ap1 ap2 lof tlof (rest rlof))))))
...
;;; STRATEGY: using HOF andmap on lof
